...
- **Does the Operator work with Terraform Enterprise / TFE?**
  ...
  Yes, the operator can be configured to use the custom TFE API endpoint using the [`operator.tfeAddress`](https://github.com/hashicorp/terraform-cloud-operator/blob/main/charts/terraform-cloud-operator/values.yaml#L28-L29) value in the Helm chart. This value should be a valid URL including the protocol(`https://`), for the API of a Terraform Enterprise instance. Once the `operator.tfeAddress` attribute is set, the operator will no longer access the public Terraform Cloud, but rather the private Terraform Enterprise instance.
